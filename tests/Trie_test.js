...
describe('TDD with Trie', () => {
  ...
  it('should assign the completeWord value to the last index of the word', function () {
    let trie = new Trie ();
    trie.insert('world')
    expect(trie.head['w'].children['o'].children['r'].children['l'].completeWord).to.deep.equal(null)
    expect(trie.head['w'].children['o'].children['r'].children['l'].children['d'].completeWord).to.deep.equal('world')
  })
